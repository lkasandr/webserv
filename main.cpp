...
// 		if (pfds[i].revents & POLLIN)  // проверка входящего события
// 		{
//             if (check_fd(sockets, pfds[i].fd))		// проверяем в каком сокете произошло событие
//                 accept_connection(sockets, pfds, pfds[i].fd);
//             else
//             {
//                 char *buffer = new char[25000];
// 				int bytes_received = recv(pfds[i].fd, buffer, 25000, 0); // считываем входящее сообщение
//                 if (bytes_received <= 0)
//                 {
// 					close(pfds[i].fd);
